...
router.post('/', (req, res) => {
    try {
        ...
        connection.query(`insert into products(title, discount, price, rating, details, qty, category, type, brand) values(?,?,?,?,?,?,?,?,?)`, 
        ...
        else if(file.size > process.env.FILE_SIZE)
{
    res.status(400).json({msg: `File size greater than 5MB`});
            ...
            res.json({msg: `${process.env.FILE_PATH}/${file.name}`})
          });
        }
    } catch (err) {
        console.log(err);
...
router.get('/', (req, res) => {
    try {
        connection.query(`select * from products order by createdAt desc`, (fetchProductsErr, fetchProductsRes) => {
            if(fetchProductsErr) {
...
router.get('/:id', (req, res) => {
    try {
        connection.query(`select * from products where id = ?`, [req.params.id], (fetchProductErr, fetchProductRes) => {
            if(fetchProductErr) {
...
router.put('/:id', (req, res) => {
    try {
        ...
        connection.query(`update products set title = ?, discount = ?, price = ?, rating = ?, details = ?, qty = ?, category = ? , type = ? , brand = ?  where id = ?`, 
        [title, discount,price, rating, details, qty, category, type, brand, req.params.id],
...
router.delete('/:id', (req, res) => {
    try {
        connection.query(`delete from products where id = ?`, [req.params.id], (deleteProductErr, deleteProductRes) => {
            if(deleteProductErr) {
